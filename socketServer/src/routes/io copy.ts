...
export const routesIo = (io: Server) => {
    io.on('connection', function (socket) {
        ...
        let room: string = query.room as string
        console.log(username + '连接了')
        if (clients.some(v => v.userId === socket.id)) {
          return
        }
        socket.join(room)
        clients.push({userId: socket.id, username, video: false, audio: false})
        if (clients.length >= 2) {
          io.sockets.in(room).emit('ready')
        }
        console.log("222")
        socket.emit('joined', "", ()=>{console.log("fsldfjsidj")})
        socket.broadcast.to(room).emit('join', {username})
        io.sockets.in(room).emit('clients', clients)


        socket.on("ready", function ({video, audio}) {
          let tmp = clients.find(item=>item.userId===socket.id)
          tmp.video = video
          tmp.audio = audio
          io.sockets.in(room).emit('clients', clients)
        })
        socket.on("ice", function(data){
          socket.to(data.user_id).emit(data)
        })
