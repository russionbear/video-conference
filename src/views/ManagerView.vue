...
const createPeerConnection = (user_id: string) => {
  ...
  if (pc_cache[username] === undefined) {
    pc = new RTCPeerConnection(PeerConnection);
  } else {
...
onMounted(async () => {
  ...
  // audio_stream.value = await navigator.mediaDevices.getUserMedia({audio: true})

  // audio_stream.value.getVideoTracks().forEach(item=>item.enabled)
